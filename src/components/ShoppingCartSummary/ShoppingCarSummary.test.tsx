...
describe(ShoppingCartSummary.name, () => {
  it("should render", () => {
    ...
    expect(
      screen.getByText("Pris i alt uden rabat: 0,00 kr.")
    ).toBeInTheDocument();
    expect(
      screen.getByText("Rabat på enkelte varer: -0,00 kr.")
    ).toBeInTheDocument();
    expect(
      screen.getByText("10% rabat på ordrer over 300 kr.: -0,00 kr.")
    ).toBeInTheDocument();
    expect(screen.getByText("Total beløb: 0,00 kr.")).toBeInTheDocument();
  });
});
